...
class Experiment(): # define a new class
    ...
    def generateDB(self):
        ...
        cref = Cell(T=300,Ndop=1E15,type=self.parameters['type']) # create a cell object of temperature 300K and n typed doping 1e15 cm-3
        cellDB = [cref.changeT(T).changeNdop(Ndop) for (T,Ndop) in zip(self.parameters['temperature'],self.parameters['doping'])] # change the temperature and dopiong levels the cell into the setting.
